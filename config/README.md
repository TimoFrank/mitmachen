...
Technisch gibt es derzeit genau zwei benötigte GitHub-Environments:
`github-pages` für die öffentliche Demo und `pre-gematik` für die manuell
freigegebene GKE-Pre-Integration. Der einfache Freitagsworkflow benötigt kein
weiteres Environment, weil er nur einen Draft-PR vorbereitet. Das weiterhin im
optionalen Publish-Workflow referenzierte Environment `release-signing` wird
erst provisioniert, wenn diese umfassende Automatisierung später bewusst
aktiviert werden soll. Bis dahin erfolgen Signatur und Veröffentlichung als
getrennter manueller Vorgang. Neue
