...
- Befund: `app/versorgungs-kompass.html` enthaelt zuerst viele lokale Komponentenstile und ab ca. Zeile 8244 eine `Design system consolidation layer`, die per spaetem CSS normalisiert.
...
- Naechster Schritt: Token- und Primitive-CSS auslagern, alte Varianten abbauen, `docs/` danach regenerieren/spiegeln.
...
- Befund: `map/versorgungs-kompass-map.html` hat eigene Tokens und am Ende eigene Normalisierung mit `!important`.
...
- Befund: `login/login.html` nutzt eigene Tokens (`--page`, `--blue`, `--orange`, `--deep`), grosse Card-Radien und starken Schatten.
