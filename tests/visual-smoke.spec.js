...
async function installTargetLikeCleanUrlShell(page) {
  await page.route("**/*", async (route) => {
    ...
    const sourcePath = ["/onboarding", "/versorgung", "/versorgung/karte"].includes(pathname)
...
async function expandStakeholderOrganizationSubnavIfNeeded(page) {
  await expandSidebarSectionIfNeeded(page, "stakeholders");
  const subnav = page.locator('[data-sidebar-subnav="stakeholder-organizations"]');
  const toggle = subnav.locator(".sidebar-subnav__toggle");
  if (!await subnav.evaluate((element) => element.open)) await toggle.click();
  await expect(subnav).toHaveJSProperty("open", true);
  await expect(toggle).toHaveAttribute("aria-expanded", "true");
}

async function expandStakeholderPersonSubnavIfNeeded(page) {
  await expandSidebarSectionIfNeeded(page, "stakeholders");
  const subnav = page.locator('[data-sidebar-subnav="stakeholder-people"]');
  const toggle = subnav.locator(".sidebar-subnav__toggle");
  if (!await subnav.evaluate((element) => element.open)) await toggle.click();
  await expect(subnav).toHaveJSProperty("open", true);
  await expect(toggle).toHaveAttribute("aria-expanded", "true");
...
async function expectSingleVisiblePatientSidebarEntry(page) {
  await expandStakeholderPersonSubnavIfNeeded(page);
...
// Ausschließlich fiktive Eingaben: Diese Fixture simuliert dokumentierte Quellenarten,
// damit Vergleichslogik geprüft wird, ohne synthetische Demo-Fälle umzuklassifizieren.
function createHospitationComparisonFixture() {
  const fixture = createProtectedBackendFixture();
  fixture.hospitations = fixture.hospitations.slice(0, 4);
  const observations = [];
  const addObservation = (hospitationIndex, entry) => {
    const id = `comparison-observation-${observations.length + 1}`;
    observations.push({
      id,
      hospitationId: fixture.hospitations[hospitationIndex].id,
      title: `Testbeobachtung ${observations.length + 1}`,
      observed: "Fiktive Testnotiz: Eine konkrete Handlung wird mit Zeitpunkt dokumentiert.",
      description: "Fiktive Testnotiz: Eine konkrete Handlung wird mit Zeitpunkt dokumentiert.",
      processPhase: "Abklärung",
      problemType: "Kein Hindernis",
      observationType: "Gelungener Ablauf",
      evidenceType: "directly_observed",
      sourceReference: `Fiktive Testfeldnotiz ${id}, 09:20 Uhr`,
      status: "active",
  ...
  [
    "Befund wird im Termin gemeinsam geöffnet",
    "Arztbrief liegt bei der Abklärung vor",
    "Laborwert wird mit der Patientin besprochen",
    "Medikationsliste wird gemeinsam abgeglichen"
  ].forEach((title, index) => addObservation(index, {
    title,
    evidenceType: ["directly_observed", "reported", "source_bound", "directly_observed"][index]
  }));
  [
    ["Aufnahme", "Information fehlt"],
    ["Abklärung", "Übernahme nötig"],
    ["Versorgung", "Technik gestört"],
    ["Übergang", "Abstimmung unklar"],
    ["Nachsorge", "Verständnis erschwert"],
    ["Zugang", "Kapazität fehlt"],
    ["Versorgung", "Information fehlt"]
  ].forEach(([processPhase, problemType], index) => {
    [0, 1].forEach((hospitationIndex) => addObservation(hospitationIndex, {
      title: `Hindernis-Testfall ${index + 1}, Termin ${hospitationIndex + 1}`,
      observationType: "Hindernis", processPhase, problemType
    }));
  });
  // Gleiche Codes reichen nicht: Jede dieser Doppelungen muss ausgeschlossen bleiben.
  [
    { title: "Ausgeschlossen: Annahme", evidenceType: "interpreted" },
    { title: "Ausgeschlossen: synthetisches Beispiel", evidenceType: "synthetic_source_based" },
    { title: "Ausgeschlossen: Quellenbezug fehlt", sourceReference: "" },
    { title: "Ausgeschlossen: Quellenart fehlt", evidenceType: "" },
    { title: "Ausgeschlossen: ursprüngliche synthetische Herkunft", originalEvidenceType: "synthetic_source_based" },
    { title: "Ausgeschlossen: synthetischer Quellentyp", sourceType: "synthetic_demo_scenario" },
    { title: "Ausgeschlossen: widersprüchliche Einordnung", observationType: "Hindernis" }
  ].forEach((entry) => [0, 1].forEach((hospitationIndex) => addObservation(hospitationIndex, entry)));
  [0, 1].forEach((index) => addObservation(0, {
    title: `Einzeltermin-Wiederholung ${index + 1}`,
    processPhase: "Übergreifend", problemType: "Übernahme nötig", observationType: "Hindernis"
  }));
  fixture.hospitationObservations = observations;
  fixture.hospitations = fixture.hospitations.map((hospitation) => ({
    ...hospitation,
    documentationSummary: "Fiktive Testdokumentation für den qualitativen Fallvergleich.",
    documentationOutcome: JSON.stringify({
      kind: "hospitation-documentation-v2",
      version: 2,
      observations: observations.filter((observation) => observation.hospitationId === hospitation.id)
    })
  }));
  return fixture;
}
    ...
    (() => {
      ...
      const contacts = [
        ...
        },
        {
          ...
          mitmachenConsentTextVersion: "mitmachen-altbestand-v1",
          mitmachenConsentRecordedBy: adminId,
...
test("Kontakte: Liste und Filtertoolbar rendern", async ({ page }, testInfo) => {
  ...
  expect(careTabOrder).toEqual(["Übersicht", "Karte", "Kontakte", "Organisationen", "Auswertung", "Aktivitäten"]);
...
test("Kontakte: Einwilligungsspalte bewertet, filtert und sortiert nachvollziehbar", async ({ page }, testInfo) => {
  ...
  await expect(availableBadge).toContainText("#Mitmachen erlaubt");
  ...
  await expect(rows.locator('[data-consent-availability="available"]')).toHaveCount(1);
...
test("Kontakte: Einwilligungsbadge bleibt mobil sichtbar", async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#contact-list .mobile-contact-card[data-id="consent-granted-valid"] [data-consent-availability]')).toContainText("#Mitmachen erlaubt");
...
test("Hospitation: Framework-Modul rendern", async ({ page }, testInfo) => {
  ...
  await expect(frameworkProcessSteps.nth(1).locator(".hospitation-dashboard-funnel-badge")).toHaveText("0");
  ...
  await expect(frameworkView).toContainText("Auswirkung");
  await expect(frameworkView).toContainText("Die Codierung ist freiwillig");
...
test("Hospitation: Beobachtungsseite zeigt das Framework mit Beobachtungsfokus", async ({ page }, testInfo) => {
  ...
  await expect(reminder.locator("[data-hospitation-framework-count]")).toHaveText(["39", "0", "0", "0"]);
...
test("Hospitation: Synthetische Beispiele erzeugen keine automatischen Vergleichshinweise", async ({ page }) => {
  ...
  await expect(panel).toBeVisible();
  await expect(panel.locator("[data-hospitation-framework-count]")).toHaveText(["39", "0", "0", "0"]);
  await expect(panel.locator("[data-hospitation-pattern]")).toHaveCount(0);
  await expect(panel.locator(".hospitation-patterns-empty")).toContainText("Noch keine Vergleichshinweise");
  await expect(panel).toContainText("mindestens zwei Hospitationen");
});

test("Hospitation: Musterseite zeigt alle quellenbezogenen Vergleichshinweise", async ({ page }, testInfo) => {
  const backendFixture = createHospitationComparisonFixture();
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#hospitations:patterns", { backendFixture });
  ...
  await expect(patternFramework.locator("[data-hospitation-framework-count]")).toHaveText(["34", "8", "0", "0"]);
  ...
  await expect(patternTableHead.locator("span:not([aria-hidden])")).toHaveText(["ID", "Vergleichshinweis", "Codierung", "Beobachtungen", "Hospitationen"]);
  if (testInfo.project.name.includes("mobile")) {
  ...
  const examplePattern = cards.filter({ hasText: "Gelungener Ablauf: Kein Hindernis · Abklärung" });
  ...
  await expect(exampleCoding.locator(".hospitation-dashboard-derived-code-badge--problem")).toHaveText("Kein Hindernis");
  await expect(exampleCoding.locator(".hospitation-dashboard-derived-code-badge--phase")).toHaveText("Abklärung");
  ...
  await expect(examplePattern.getByText("Gelungener Ablauf: Kein Hindernis · Abklärung", { exact: true })).toHaveCount(1);
  ...
  await expect(filteredPanel).not.toContainText("Hindernis-Testfall");
  await expect(filteredPanel).not.toContainText("Ausgeschlossen:");
  await expect(panel).not.toContainText("Einzeltermin-Wiederholung");
  await expect(cards.locator(".hospitation-pattern-card__rank")).toHaveText(["V01", "V02", "V03", "V04", "V05", "V06", "V07", "V08"]);
...
test("Hospitation: Fragebogen-Modul rendern", async ({ page }, testInfo) => {
  ...
  await expect(questionnaireObservationStep.locator('select[name="questionnaireObservations[1][evidenceType]"]')).toHaveCount(0);
  ...
  const codingSelect = (key) => questionnaireCodingStep.locator(`select[name="questionnaireObservations[1][${key}]"]`);
  for (const key of ["processPhase", "problemType", "impact", "observationType", "evidenceType"]) {
    await expect(codingSelect(key)).toHaveValue("");
    await expect(codingSelect(key)).not.toHaveAttribute("required", "");
  }
  await expect(codingSelect("processPhase").locator("option")).toHaveText([
    "Noch offen", "Zugang", "Aufnahme", "Abklärung", "Versorgung", "Übergang", "Nachsorge", "Übergreifend", "Noch nicht zuordenbar"
  ]);
  await expect(codingSelect("problemType").locator("option")).toHaveText([
    "Noch offen", "Fehlende Information", "Doppelte Dokumentation", "Technische Störung", "Unklare Abstimmung", "Verständnisproblem", "Fehlende Kapazität", "Anderes Problem", "Kein Problem erkennbar", "Noch nicht zuordenbar"
  ]);
  await expect(codingSelect("impact").locator('option[value="Fehler"]')).toHaveCount(1);
  await expect(codingSelect("observationType").locator('option[value="Kontext"]')).toHaveCount(1);
  await expect(codingSelect("evidenceType").locator("option")).toHaveText([
    "Quelle noch offen", "direkt beobachtet", "berichtet", "Beobachtungsunterlage", "Annahme", "synthetisches Beispiel"
  ]);
  await expect(questionnaireCodingStep.locator('select[name="questionnaireObservations[1][relevance]"]')).toHaveCount(0);
  const codingGuide = questionnaireCodingStep.locator("[data-hospitation-codebook-guide]");
  ...
  await expect(codingCards.locator("[data-hospitation-journey]")).toHaveCount(0);
  await expect(codingCards.locator("[data-codebook-selected-help]:not(.visually-hidden)")).toHaveCount(0);
  const patientJourney = codingGuide.locator("[data-hospitation-journey]");
  await expect(patientJourney).toBeHidden();
  await codingGuide.locator(":scope > summary").click();
  await expect(patientJourney).toBeVisible();
  await expect(patientJourney.locator("li > strong")).toHaveText(["Zugang", "Aufnahme", "Abklärung", "Versorgung", "Übergang", "Nachsorge"]);
  await expect(patientJourney.locator('[aria-current="step"]')).toHaveCount(0);
  await codingGuide.locator(":scope > summary").click();
  const additionalCodingFields = codingCards.first().locator("[data-codebook-additional]");
  ...
  // Eine vollständige Beschreibung darf auch ohne Codes und Quellenart weitergeführt werden.
  await completeQuestionnaireStep(questionnaireCodingStep);
  await expect(questionnaireMaterialStep).toHaveAttribute("open", "");
  if (testInfo.project.name.includes("mobile")) {
    await page.locator("[data-questionnaire-step-previous]").click();
  } else {
    await questionnaireCodingStep.locator(":scope > summary").click();
  }
  await expect(questionnaireCodingStep).toHaveAttribute("open", "");
  await codingSelect("processPhase").selectOption("Aufnahme");
  await codingSelect("problemType").selectOption("Information fehlt");
  await codingSelect("impact").selectOption("Zusätzliche Arbeit");
  await additionalCodingFields.locator(":scope > summary").click();
  await codingSelect("observationType").selectOption("Hindernis");
  await codingSelect("evidenceType").selectOption("directly_observed");
  await questionnaireCodingStep.locator('textarea[name="questionnaireObservations[1][sourceReference]"]').fill("Fiktive Testfeldnotiz, 09:20 Uhr");
  await questionnaireCodingStep.locator('textarea[name="questionnaireObservations[1][immediateConsequence]"]').fill("Die MFA ruft die Praxis an und unterbricht ihre Vorbereitung.");
  await expect(patientJourney.locator('[aria-current="step"]')).toHaveCount(0);
  await completeQuestionnaireStep(questionnaireCodingStep);
  await expect(questionnaireMaterialStep).toHaveAttribute("open", "");
  ...
  await expect(questionnaireDrawerObservation.locator('[data-repeatable-field="processPhase"]')).toHaveValue("Aufnahme");
  ...
  await expect(questionnaireDrawerObservation.locator('[data-repeatable-field="relevanceScore"]')).toHaveValue("");
  await expect(questionnaireDrawerObservation.locator('[data-repeatable-field="evidenceType"]')).toHaveValue("directly_observed");
  await expect(questionnaireDrawerObservation.locator('[data-repeatable-field="sourceReference"]')).toHaveValue("Fiktive Testfeldnotiz, 09:20 Uhr");
  await expect(questionnaireDrawerObservation.locator('[data-repeatable-field="immediateConsequence"]')).toHaveValue("Die MFA ruft die Praxis an und unterbricht ihre Vorbereitung.");
...
test("Hospitationen: geschützte synthetische Backend-Fixture ist observation-first und bewertungsfrei", async ({ page }) => {
  ...
  await expect(first.locator(".hospitation-observation-advanced")).not.toHaveAttribute("open", "");
  await expect(first.locator('[data-repeatable-field="immediateConsequence"]')).toBeHidden();
  await expect(first.locator('[data-repeatable-field="sourceReference"]')).toBeHidden();
  await expect(first.locator('[data-repeatable-field="uncertainty"]')).toBeHidden();
  await expect(first.locator('[data-repeatable-field="relevanceScore"]')).toBeHidden();
  ...
  for (const [key, value] of [["processPhase", "Befund / Dokumentation"], ["problemType", "fehlende Information"], ["impact", "Fehleranfälligkeit"]]) {
    const control = first.locator(`[data-repeatable-field="${key}"]`);
    await expect(control).toHaveValue(value);
    await expect(control.locator("option:checked")).toHaveText(`${value} (bisherige Codierung)`);
  }
  await expect(first.locator('[data-repeatable-field="processPhase"] option[value="Aufnahme"]')).toHaveCount(1);
  await expect(first.locator('[data-repeatable-field="processPhase"] option[value="Anmeldung / Aufnahme"]')).toHaveCount(0);
...
test("Sidebar: Aktiver Bereich startet offen und beim Modulwechsel bleibt nur ein Bereich offen", async ({ page }, testInfo) => {
  ...
  await expandStakeholderOrganizationSubnavIfNeeded(page);
...
test("Sidebar: Stakeholder-Personen und -Organisationen sind kompakt und routensicher gruppiert", async ({ page }, testInfo) => {
  ...
  const shell = page.locator(".app-shell");
  const stakeholderSection = page.locator('[data-sidebar-section="stakeholders"]');
  const personSubnav = stakeholderSection.locator('[data-sidebar-subnav="stakeholder-people"]');
  const personToggle = personSubnav.locator(".sidebar-subnav__toggle");
  const visiblePersonDestinations = personSubnav.locator("[data-view-tab]:visible");
  const organizationSubnav = stakeholderSection.locator('[data-sidebar-subnav="stakeholder-organizations"]');
  const organizationToggle = organizationSubnav.locator(".sidebar-subnav__toggle");
  const visibleOrganizationDestinations = organizationSubnav.locator("[data-stakeholder-type-route]:visible");

  await expect(stakeholderSection).toHaveClass(/is-expanded/);
  await expect(personSubnav).toHaveJSProperty("open", false);
  await expect(personToggle).toHaveAttribute("aria-expanded", "false");
  await expect(visiblePersonDestinations).toHaveCount(0);
  await expect(organizationSubnav).toHaveJSProperty("open", false);
  await expect(organizationToggle).toHaveAttribute("aria-expanded", "false");
  await expect(visibleOrganizationDestinations).toHaveCount(0);
  const personToggleBox = await personToggle.boundingBox();
  const organizationToggleBox = await organizationToggle.boundingBox();
  expect(personToggleBox).not.toBeNull();
  expect(organizationToggleBox).not.toBeNull();
  expect(personToggleBox.height).toBeGreaterThanOrEqual(isMobile ? 44 : 38);
  expect(organizationToggleBox.height).toBeGreaterThanOrEqual(isMobile ? 44 : 38);

  await personToggle.click();
  await expect(personSubnav).toHaveJSProperty("open", true);
  await expect(personToggle).toHaveAttribute("aria-expanded", "true");
  await expect(visiblePersonDestinations).toHaveCount(4);
  if (isMobile) {
    expect(await visiblePersonDestinations.evaluateAll((destinations) =>
      destinations.every((destination) => destination.getBoundingClientRect().height >= 44)
    )).toBe(true);
  }

  const politics = personSubnav.locator('[data-view-tab="politics"]');
  await politics.click();
  await expect(page).toHaveURL(/#politics$/);
  await expect(personSubnav).toHaveClass(/is-active-subnav/);
  await expect(personSubnav).toHaveJSProperty("open", true);
  await expect(politics).toHaveAttribute("aria-current", "page");
  if (isMobile) await openMobileSidebarIfNeeded(page);

  await organizationToggle.click();
  await expect(organizationSubnav).toHaveJSProperty("open", true);
  await expect(organizationToggle).toHaveAttribute("aria-expanded", "true");
  await expect(visibleOrganizationDestinations).toHaveCount(4);
  await expect(personSubnav).toHaveJSProperty("open", false);
  await expect(visiblePersonDestinations).toHaveCount(0);

  const healthInsurance = organizationSubnav.locator('[data-stakeholder-type-route="health-insurance"]');
  await healthInsurance.click();
  await expect(page).toHaveURL(/#stakeholders\/krankenkassen$/);
  await expect(organizationSubnav).toHaveClass(/is-active-subnav/);
  await expect(organizationSubnav).toHaveJSProperty("open", true);
  await expect(healthInsurance).toHaveAttribute("aria-current", "page");
  ...
  await page.locator("#sidebar-collapse-button").click();
  await expect(shell).toHaveClass(/is-sidebar-collapsed/);
  await expect(personToggle).toBeHidden();
  await expect(organizationToggle).toBeHidden();
  await expect(visiblePersonDestinations).toHaveCount(4);
  await expect(visibleOrganizationDestinations).toHaveCount(4);
  await expect(stakeholderSection.locator(":scope > .sidebar-section-toggle .sidebar-module-icon img")).toBeVisible();
});
...
test("Organisationen: Demo-Daten rendern im CRM-Profilmodus", async ({ page }, testInfo) => {
  ...
  if (isMobile) {
    await expect(firstOrganization).toHaveClass(/mobile-contact-card/);
    await expect(firstOrganization).toHaveClass(/mobile-collection-card/);
    await expect(firstOrganization.locator(".mobile-contact-top")).toBeVisible();
    await expect(firstOrganization.locator(".mobile-contact-copy .mobile-contact-name")).toBeVisible();
    await expect(firstOrganization.locator(".mobile-contact-chevron")).toBeVisible();
    await expect(firstOrganization.locator("[data-contextual-row-select]")).toHaveCount(0);
  ...
  await expect(page.locator("#search")).toBeVisible();
  await expect(page.locator("#organization-table-toolbar #organization-matching-worklist-button")).toHaveCount(0);
  await expect(page.locator("#organization-secondary-actions #organization-matching-worklist-button")).toContainText("Dubletten");
...
test("Expertenkreis: getrennte Kontakt- und Organisationsansicht rendert", async ({ page }, testInfo) => {
  ...
  await expect(page.locator("#expert-table-toolbar > #expert-mode-actions")).toBeVisible();
  await expect(page.locator("#search")).toBeVisible();
  await expect(page.locator(".controls")).toBeVisible();
  await expect(page.locator(".controls #search")).toHaveCount(1);
  await expect(page.locator("#expert-table-toolbar > #expert-mode-actions .experts-mode-count")).toHaveCount(2);
...
test("Patienten: Organisationsliste nach Indikation rendert ohne Kontakte", async ({ page }, testInfo) => {
  ...
  expect(patientModeContract).toEqual([
    ...
    { mode: "organizations", label: "Organisationen" },
  ...
  if (mobileProject) {
    const patientTabsStayInsideButtons = await page.locator("#patient-mode-actions [data-patient-mode]").evaluateAll((tabs) =>
      tabs.every((tab) => {
        ...
        return [...tab.children].every((child) => {
          if (child.getClientRects().length === 0) return true;
          const childBounds = child.getBoundingClientRect();
          return childBounds.left >= tabBounds.left - 1 && childBounds.right <= tabBounds.right + 1;
        });
      })
    );
    expect(patientTabsStayInsideButtons).toBe(true);
    const patientModeCounts = page.locator("#patient-mode-actions .experts-mode-count");
    await expect(patientModeCounts).toHaveCount(3);
    expect(await patientModeCounts.evaluateAll((counts) =>
  ...
  const organizationIndicationBadges = mobileProject
    ? page.locator("#patient-organization-list .mobile-collection-tags .patient-indication-badge")
  ...
  if (testInfo.project.name.includes("mobile")) {
    ...
    await expect(page.locator('#organization-profile-body [data-detail-tab="people"]')).toContainText("Kontakte (1)");
    await page.locator('#organization-profile-body [data-detail-tab="people"]').click();
    await expect(page.locator("#organization-profile-body #patient-organization-people")).toContainText("Demo-Patientenkontakt 01");
  } else {
    ...
    await expect(page.locator('#detail-drawer [data-detail-tab="people"]')).toContainText("Kontakte (1)");
    await page.locator('#detail-drawer [data-detail-tab="people"]').click();
    await expect(page.locator("#detail-drawer #patient-organization-people")).toContainText("Demo-Patientenkontakt 01");
  }
...
test("Dubletten: Admin-Ansichten bleiben im jeweiligen Tab", async ({ page }, testInfo) => {
  ...
  await expect(page.locator("#organization-table-toolbar #organization-matching-worklist-button")).toHaveCount(0);
  await expect(page.locator("#organization-secondary-actions #organization-matching-worklist-button")).toBeVisible();
...
test("Kontaktprofil: Detailpanel oeffnet im Lesemodus", async ({ page }, testInfo) => {
  ...
  if (isMobile) {
    ...
    expect(drawerMetrics.scrolledPanel).toBeGreaterThanOrEqual(drawerMetrics.initialPanelScroll);
    if (drawerMetrics.canScrollPanel) {
...
test("Kontaktprofil: bestehende Hospitation öffnet direkt; Gesamtliste übernimmt Kontaktfilter", async ({ page }, testInfo) => {
  ...
  await documented.getByRole("button", { name: "Öffnen", exact: true }).click();
...
test("Kontaktprofil: Einwilligungen sind zweckbezogen und per Tastatur dokumentierbar", async ({ page }, testInfo) => {
  const consentPatches = [];
  page.on("request", (request) => {
    if (request.method() !== "PATCH" || !request.url().endsWith("/api/contacts/consent-not-requested")) return;
    consentPatches.push(request.postDataJSON());
  });
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#contacts", {
    role: "admin",
    backendFixtureScript: consentBackendFixtureScript()
  ...
  const statusRows = root.locator(".consent-status-row");
  ...
  await expect(root.locator(".consent-scope-summary")).toContainText("#Mitmachen-Nachrichten nicht erlaubt");
  await expect(root.locator(".consent-disclosure")).toHaveCount(1);
  await expect(root.getByRole("button", { name: "Einwilligungen bearbeiten" })).toHaveCount(1);
  await expect(root.locator(".consent-overview-editor:visible")).toHaveCount(0);
  const statusGeometry = await statusRows.evaluateAll((rows) => rows.map((row) => {
    const box = row.getBoundingClientRect();
    const icon = row.querySelector(".consent-status-row__icon")?.getBoundingClientRect();
    return { top: box.top, bottom: box.bottom, iconLeft: icon?.left || 0 };
  }));
  expect(statusGeometry[1].top).toBeGreaterThanOrEqual(statusGeometry[0].bottom - 1);
  expect(statusGeometry[2].top).toBeGreaterThanOrEqual(statusGeometry[1].bottom - 1);
  expect(Math.max(...statusGeometry.map((item) => item.iconLeft)) - Math.min(...statusGeometry.map((item) => item.iconLeft))).toBeLessThanOrEqual(1);
  await attachScreenshot(page, testInfo, "kontaktprofil-einwilligungen-ampel", { fullPage: false });

  const editButton = root.getByRole("button", { name: "Einwilligungen bearbeiten" });
  await editButton.click();
  const statusForm = root.locator("#consent-overview-editor");
  await expect(editButton).toHaveAttribute("aria-expanded", "true");
  await expect(root.locator('[data-consent-disclosure="evidence"]')).not.toHaveAttribute("open", "");
  await expect(statusForm).toBeVisible();
  await expect(statusForm.locator("fieldset")).toHaveCount(3);
  await expect(statusForm.locator('input[name="relationship_basis"]:checked')).toBeFocused();
  await expect(statusForm.locator('button[type="submit"]')).toHaveCount(1);
  await statusForm.locator('button[type="submit"]').click();
  await expect(statusForm.locator("[data-consent-status]")).toHaveText("Keine Änderungen.");
  expect(consentPatches).toHaveLength(0);
  await page.keyboard.press("Escape");
  await expect(statusForm).toBeHidden();
  await expect(editButton).toBeFocused();

  await editButton.click();
  await statusForm.locator('input[name="mitmachen_status"][value="granted"]').check();
  await statusForm.locator('select[name="mitmachen_source"]').selectOption("manual_transfer");
  await expect(statusForm.locator('input[name="mitmachen_effectiveAt"]')).not.toHaveValue("");
  await statusForm.locator('input[name="mitmachen_effectiveAt"]').fill("2026-07-16T10:00");
  const note = statusForm.locator('textarea[name="mitmachen_note"]');
  await expect(note).toBeVisible();
  await note.press("Control+Enter");
  await expect(statusForm).toBeVisible();
  await expect(statusForm.locator("[data-consent-status]")).toContainText("Kurzer Nachweis fehlt");
  await expect(note).toBeFocused();

  await note.fill("Am 16.07.2026 aus dem freigegebenen Altbestand nachvollziehbar übernommen.");
  await note.press("Control+Enter");
  await expect(root.locator('[data-consent-signal="mitmachen"]')).toContainText("Übernommen · prüfen");
  await expect(root.getByRole("button", { name: "Einwilligungen bearbeiten" })).toBeFocused();
  expect(consentPatches).toHaveLength(1);
  expect(Object.keys(consentPatches[0])).toEqual(expect.arrayContaining([
  ...
  expect(Object.keys(consentPatches[0]).some((key) => key.startsWith("ehc") || key.startsWith("relationship"))).toBe(false);
  ...
  await root.locator('[data-detail-tab="consent"]').click();
  await root.getByRole("button", { name: "Einwilligungen bearbeiten" }).click();
  const withdrawalForm = root.locator("#consent-overview-editor");
  await withdrawalForm.locator('input[name="mitmachen_status"][value="withdrawn"]').check();
  await expect(withdrawalForm.locator('input[name="mitmachen_effectiveAt"]')).not.toHaveValue("2026-07-16T10:00");
  await withdrawalForm.locator('textarea[name="mitmachen_note"]').fill("Widerruf am 28.07.2026 dokumentiert.");
  ...
  expect(withdrawalDialog).toContain("nicht mehr angesprochen");
  expect(consentPatches).toHaveLength(2);
  expect(consentPatches[1]).toMatchObject({
    mitmachenConsentStatus: "withdrawn",
    mitmachenConsentNote: "Widerruf am 28.07.2026 dokumentiert."
  });
  expect(consentPatches[1].mitmachenConsentEffectiveAt).not.toBe(consentPatches[0].mitmachenConsentEffectiveAt);
  expect(consentPatches[1]).not.toHaveProperty("mitmachenConsentSource");
  expect(consentPatches[1]).not.toHaveProperty("mitmachenConsentTextVersion");
  await expect(root.locator(".consent-scope-summary")).toContainText("Nicht kontaktieren");
  await root.locator('[data-consent-disclosure="evidence"] > summary').click();
  await expect(root.locator('[data-consent-record="mitmachen"]')).toContainText("Manuelle Übernahme");
  await expect(root.locator('[data-consent-record="mitmachen"]')).toContainText("mitmachen-kontakt-v1");
  await expect(root.locator('[data-consent-record="mitmachen"]')).toContainText("Widerruf am 28.07.2026 dokumentiert.");
...
test("Kontaktprofil: gemeinsamer Einwilligungseditor lässt sich einfach öffnen und abbrechen", async ({ page }, testInfo) => {
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#contacts", {
    role: "editor",
    backendFixtureScript: consentBackendFixtureScript()
  });
  ...
  const editButton = root.getByRole("button", { name: "Einwilligungen bearbeiten" });
  await editButton.click();
  const form = root.locator("#consent-overview-editor");
  await expect(form).toBeVisible();
  await expect(form.getByRole("group", { name: "Profil im Versorgungskompass" })).toBeVisible();
  await expect(form.getByRole("group", { name: "E-Health Community" })).toBeVisible();
  await expect(form.getByRole("group", { name: "#Mitmachen" })).toBeVisible();
  await expect(form.locator('input[name="relationship_basis"]:checked')).toBeFocused();
  await form.getByRole("button", { name: "Abbrechen" }).click();
  await expect(form).toBeHidden();
  await expect(editButton).toBeFocused();
});

test("Kontaktprofil: Vermerkänderung erhält vorhandene Einwilligungsnachweise", async ({ page }, testInfo) => {
  const consentPatches = [];
  page.on("request", (request) => {
    if (request.method() !== "PATCH" || !request.url().endsWith("/api/contacts/consent-clarification")) return;
    consentPatches.push(request.postDataJSON());
  });
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#contacts", {
    role: "admin",
  ...
  const isMobile = testInfo.project.name.includes("mobile");
  await page.locator('#contact-list [data-id="consent-clarification"]').click();
  const root = page.locator(isMobile ? "#person-profile-body" : "#detail-drawer");
  await root.locator('[data-detail-tab="consent"]').click();
  await root.getByRole("button", { name: "Einwilligungen bearbeiten" }).click();
  const form = root.locator("#consent-overview-editor");
  await expect(form.locator('select option[value="verbal_confirmed"]')).toHaveCount(0);
  await expect(form.locator('select[name="mitmachen_source"]')).toHaveValue("manual_transfer");
  await expect(form.locator('input[name="mitmachen_textVersion"]')).toHaveValue("mitmachen-altbestand-v1");
  await form.locator('textarea[name="mitmachen_note"]').fill("Altbestand wird schriftlich geprüft.");
  await form.locator('button[type="submit"]').click();

  await expect(root.getByRole("button", { name: "Einwilligungen bearbeiten" })).toBeFocused();
  expect(consentPatches).toEqual([{ mitmachenConsentNote: "Altbestand wird schriftlich geprüft." }]);
  await root.locator('[data-consent-disclosure="evidence"] > summary').click();
  await expect(root.locator('[data-consent-record="mitmachen"]')).toContainText("Manuelle Übernahme");
  await expect(root.locator('[data-consent-record="mitmachen"]')).toContainText("mitmachen-altbestand-v1");
  await expect(root.locator('[data-consent-record="mitmachen"]')).toContainText("Altbestand wird schriftlich geprüft.");
});

test("Kontaktprofil: Öffnen erfindet keinen fehlenden Einwilligungszeitpunkt", async ({ page }, testInfo) => {
  const consentPatches = [];
  page.on("request", (request) => {
    if (request.method() !== "PATCH" || !request.url().endsWith("/api/contacts/consent-granted-invalid")) return;
    consentPatches.push(request.postDataJSON());
  });
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#contacts", {
    role: "admin",
  ...
  const isMobile = testInfo.project.name.includes("mobile");
  await page.locator('#contact-list [data-id="consent-granted-invalid"]').click();
  const root = page.locator(isMobile ? "#person-profile-body" : "#detail-drawer");
  await root.locator('[data-detail-tab="consent"]').click();
  await root.getByRole("button", { name: "Einwilligungen bearbeiten" }).click();
  const form = root.locator("#consent-overview-editor");
  await expect(form.locator('input[name="mitmachen_effectiveAt"]')).toHaveValue("");
  await form.locator('button[type="submit"]').click();
  await expect(form.locator("[data-consent-status]")).toHaveText("Keine Änderungen.");
  expect(consentPatches).toHaveLength(0);
});
...
test("Kontaktprofil: EHC-only und mündliche Nachfassung sind auf einen Blick unterscheidbar", async ({ page }, testInfo) => {
  ...
  await profile.getByRole("button", { name: "Einwilligungen bearbeiten" }).click();
  const ehcForm = profile.locator("#consent-overview-editor");
  await ehcForm.locator('input[name="ehc_status"][value="withdrawn"]').check();
  await ehcForm.locator('textarea[name="ehc_note"]').fill("EHC-Einwilligung am 28.07.2026 widerrufen.");
  page.once("dialog", (dialog) => dialog.accept());
  await ehcForm.locator('button[type="submit"]').click();
  await expect(profile.locator(".ehc-profile-badge")).toContainText("EHC geschützt");
  await expect(profile.locator(".consent-scope-summary")).toContainText("Nicht kontaktieren");
  await expect(profile.locator('[data-consent-signal="ehc"]')).toContainText("Widerrufen");
  await expect(profile.locator(".consent-protection-note")).toBeVisible();

  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#person/contact/consent-verbal?tab=consent", {
    role: "admin",
    backendFixtureScript: consentBackendFixtureScript()
  });
  await expect(profile.locator(".consent-scope-summary")).toContainText("Schriftlich nachfassen");
  await expect(profile.locator('[data-consent-signal="relationship"]')).toContainText("Mündlich dokumentiert");
  await expect(profile.locator('[data-consent-signal="mitmachen"]')).toContainText("Mündlich bestätigt");
});
...
test("Kontaktprofil: Viewer lesen Notizen-Chat ohne Composer", async ({ page }, testInfo) => {
  ...
  await expect(profile.locator(".consent-overview-editor")).toHaveCount(0);
  await expect(profile.locator("[data-edit-consent-overview]")).toHaveCount(0);
  await expect(profile.locator("#detail-consent .consent-status-list")).toBeVisible();
...
test("Hospitationen: Dokumentationsdrawer mit Reitern", async ({ page }, testInfo) => {
  ...
  await observationCards.nth(0).locator('[data-observation-coding] > summary').click();
  ...
  await expect(observationCards.nth(0).locator('[data-repeatable-field="evidenceType"]')).toHaveValue("synthetic_source_based");
  await expect(observationCards.nth(0).locator('[data-repeatable-original]')).toHaveAttribute("value", /synthetic_demo_scenario/);
  ...
  await observationCards.nth(initialObservationCount).locator('[data-observation-coding] > summary').click();
  ...
  await observationCards.nth(initialObservationCount).locator('[data-repeatable-field="evidenceType"]').selectOption("reported");
  await observationCards.nth(initialObservationCount).locator(".hospitation-observation-advanced > summary").click();
  await observationCards.nth(initialObservationCount).locator('[data-repeatable-field="sourceReference"]').fill("Fiktiver Testbericht einer MFA, 09:30 Uhr");
  await observationCards.nth(initialObservationCount).locator('[data-repeatable-field="observationType"]').selectOption("Hindernis");
  await addObservationButton.scrollIntoViewIfNeeded();
  await addObservationButton.click();
  ...
  await observationCards.nth(initialObservationCount + 1).locator('[data-repeatable-field="observed"]').fill("Die vorhandene Anschrift wird aus einem Papierformular noch einmal in ein zweites Formular eingetragen.");
  await observationCards.nth(initialObservationCount + 1).locator('[data-observation-coding] > summary').click();
  await observationCards.nth(initialObservationCount + 1).locator('[data-repeatable-field="processPhase"]').selectOption({ label: "Abklärung" });
  ...
  expect(newlySavedObservations).toEqual([
    { title: "Rückfrage ohne Zuständigkeit", evidenceType: "reported", processPhase: "Übergang", problemType: "Abstimmung unklar" },
    { title: "Formular als Medienbruch", evidenceType: "", processPhase: "Abklärung", problemType: "Doppelte Dokumentation" }
  ]);
  ...
  await expect(dashboard).toContainText("Dokumentierte Prozessphasen");
  ...
  await expect(dashboard).toContainText("Dokumentierte Hindernisse");
  ...
  await expect(dashboard).toContainText("Abstimmung unklar");
  ...
  await expect(processAccordion.locator("summary")).toContainText("Dokumentierte Prozessphasen");
  await expect(observationsAccordion.locator("summary")).toContainText("Beobachtungen");
  await expect(problemTypesAccordion.locator("summary")).toContainText("Dokumentierte Hindernisse");
...
test("Versorgungs-Kompass: Übersicht verbindet Kartenvorschau und Arbeitsbereiche", async ({ page }, testInfo) => {
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#care", { role: "admin" });
  ...
  await expect(page.locator("#care-overview-sector-count-label")).toHaveText(/Sektor(?:en)? mit Kontakten/);
  await expect(page.locator("#care-overview-activity-count")).toHaveAttribute("data-state", "ready");
  await expect(page.locator("#care-overview-activity-count-value")).toHaveText(/\d+/);
  await expect(page.locator("#care-overview-activity-count-label")).toHaveText("letzte 30 Tage");
  ...
  const compassMotif = previewFrame.locator(".care-overview-compass-motif");
  await expect(compassMotif).toBeVisible();
  await expect(compassMotif).toHaveAttribute("aria-hidden", "true");
  await expect(compassMotif.locator("svg")).toBeVisible();
  expect(await compassMotif.evaluate((node) => Number.parseInt(getComputedStyle(node).zIndex, 10))).toBe(700);
  expect(await page.evaluate(() => ({
    start: getComputedStyle(document.documentElement).getPropertyValue("--care-overview-atlas-start").trim(),
    end: getComputedStyle(document.documentElement).getPropertyValue("--care-overview-atlas-end").trim()
  }))).toEqual({ start: "#dff3ff", end: "#d9e1f8" });
  ...
  expect(Number.isFinite(mappedSummaryCount)).toBe(true);
  expect(await page.evaluate(() => globalThis.eval(
    'careOverviewMapSummary([{ id: "invalid-map-position", lat: 91, lon: 8 }]).mappedCount'
  ))).toBe(0);
  await expect.poll(() => previewFrame.locator("#map").evaluate(() =>
    globalThis.eval("currentEntries.length")
  )).toBe(mappedSummaryCount);
  await expect.poll(() => previewFrame.locator("#map").evaluate(() =>
    globalThis.eval("heatMapActive && stateHeatLayer && map.hasLayer(stateHeatLayer)")
  )).toBe(true);
  const schematicStates = previewFrame.locator(".care-overview-schematic-map__state");
  const schematicMap = previewFrame.locator(".care-overview-schematic-map");
  await expect(schematicMap).toBeVisible();
  expect(await schematicMap.evaluate((node) => Number.parseInt(getComputedStyle(node).zIndex, 10))).toBeLessThan(700);
  await expect(schematicMap).toHaveAttribute("viewBox", "0 0 652 848");
  ...
  const previewGeometryState = await schematicStates.evaluateAll((paths) => ({
    stateNames: paths.map((path) => path.dataset.state),
    referenceIds: paths.map((path) => path.dataset.referenceId),
    pathData: paths.map((path) => path.getAttribute("d"))
  }));
  expect(previewGeometryState.stateNames).toEqual([
    "Bayern", "Baden-Württemberg", "Thüringen", "Hessen", "Rheinland-Pfalz", "Nordrhein-Westfalen",
    "Niedersachsen", "Bremen", "Saarland", "Brandenburg", "Sachsen", "Sachsen-Anhalt",
    "Mecklenburg-Vorpommern", "Schleswig-Holstein", "Hamburg", "Berlin"
  ]);
  expect(previewGeometryState.referenceIds).toEqual([
    "bayern", "bawue", "thue", "hessen", "rheinpf", "nrw", "nieders", "bremen",
    "saar", "brandenburg", "sachs", "sachan", "mcpom", "schlehol", "hamburg", "berlin"
  ]);
  expect(createHash("sha256").update(previewGeometryState.referenceIds
    .map((referenceId, index) => `${referenceId}=${previewGeometryState.pathData[index]}`)
    .join("\n")).digest("hex"))
    .toBe("be97ff28fe33663eab2e813a0644a92d6f5acea8218c50f508bd9db182d19220");
  await expect(previewFrame.locator(".leaflet-map-pane")).toHaveCSS("visibility", "hidden");
  expect(await previewFrame.locator("#map").evaluate(() => globalThis.eval(`
    Array.from(document.querySelectorAll(".care-overview-schematic-map__state")).every((path) => {
  ...
  const [previewFrameBox, previewMapBox] = await Promise.all([
    page.locator("#care-overview-map-frame").boundingBox(),
    previewFrame.locator("#map").boundingBox()
  ]);
  ...
  await expect.poll(() => previewFrame.locator("#map").evaluate(() => {
    ...
    return mapBounds.contains(germanyBounds);
  })).toBe(true);
  await expectNoHorizontalOverflow(page);

  if (testInfo.project.name === "chromium-desktop") {
    const mapVisualStateBeforeHover = await mapPreview.evaluate((node) => ({
  ...
  const geometry = await overview.evaluate((panel) => {
    ...
    return {
      map: rectangle(".care-overview-map"),
      contacts: rectangle(".care-overview-destination--contacts"),
      analytics: rectangle(".care-overview-destination--analytics"),
      activities: rectangle(".care-overview-destination--activities"),
      viewportWidth: window.innerWidth
    };
  });
  if (geometry.viewportWidth > 1180) {
    expect(geometry.map.left).toBeGreaterThanOrEqual(geometry.contacts.right);
    expect(geometry.analytics.left).toBeGreaterThanOrEqual(geometry.map.right);
    const contactDestination = overview.locator(".care-overview-destination--contacts");
    const connectorOpacityBeforeHover = Number.parseFloat(await contactDestination.evaluate((node) =>
      getComputedStyle(node, "::after").opacity
    ));
    const iconTransformBeforeHover = await contactDestination.locator(".care-overview-destination__icon").evaluate((node) =>
      getComputedStyle(node).transform
    );
    await contactDestination.hover();
    await expect.poll(() => contactDestination.evaluate((node) =>
      Number.parseFloat(getComputedStyle(node, "::after").opacity)
    )).toBeGreaterThan(connectorOpacityBeforeHover);
    await expect.poll(() => contactDestination.locator(".care-overview-destination__icon").evaluate((node) =>
      getComputedStyle(node).transform
    )).not.toBe(iconTransformBeforeHover);
    await page.mouse.move(0, 0);
  } else if (geometry.viewportWidth <= 600) {
    expect(Math.max(geometry.contacts.bottom, geometry.analytics.bottom, geometry.activities.bottom))
      .toBeLessThanOrEqual(geometry.map.top);
    expect(await overview.locator("a:visible").evaluateAll((links) => links.map((link) => link.dataset.routeLink)))
      .toEqual(["contacts", "organizations", "analytics", "activities", "map"]);
  } else {
    expect(geometry.map.bottom).toBeLessThanOrEqual(geometry.contacts.top);
  }
  ...
  await expect(fullMapFrame.locator(".sidebar")).toBeVisible();
  await expect(fullMapFrame.locator(".care-overview-schematic-map")).toHaveCount(0);
  const fullMapHeatScale = await fullMapFrame.locator("#map").evaluate(() =>
    globalThis.eval("[0, 1, 2, 4].map((value) => heatColor(value, 4))")
  );
...
test("Versorgungs-Kompass: Übersicht bleibt bei reduzierter Bewegung statisch", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "chromium-desktop", "Reduced Motion wird einmal im Desktop-Projekt geprüft.");
  ...
  const motionState = await overview.locator(".care-overview-destination--contacts").evaluate((node) => {
    const cardStyle = getComputedStyle(node);
    const iconStyle = getComputedStyle(node.querySelector(".care-overview-destination__icon"));
    return {
      animationName: cardStyle.animationName,
      cardTransition: cardStyle.transitionDuration,
      iconTransition: iconStyle.transitionDuration,
    };
  });
  expect(motionState.animationName).toBe("none");
  expect(motionState.cardTransition).toBe("0s");
  expect(motionState.iconTransition).toBe("0s");
  const mapMotionState = await overview.locator(".care-overview-map").evaluate((node) => ({
    mapTransition: getComputedStyle(node).transitionDuration,
    frameTransition: getComputedStyle(node.querySelector(".care-overview-map__frame")).transitionDuration
  }));
  expect(mapMotionState.mapTransition).toBe("0s");
  expect(mapMotionState.frameTransition).toBe("0s");
  await overview.locator(".care-overview-map").hover();
  await expect(overview.locator(".care-overview-map__frame")).toHaveCSS("transform", "none");
});

test("Versorgungs-Kompass: Übersicht spiegelt die Viewer-Rechte", async ({ page }) => {
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#care", { role: "viewer" });

  const overview = page.locator('[data-view-panel="careOverview"]');
  await expect(overview).toBeVisible();
  await expect(overview.locator(".care-overview-map")).toBeVisible();
  await expect(overview.locator(".care-overview-destination:visible")).toHaveCount(2);
  await expect(overview.locator(".care-overview-destination--contacts")).toBeVisible();
  await expect(overview.locator(".care-overview-destination--organizations")).toBeVisible();
  await expect(overview.locator(".care-overview-destination--analytics")).toBeHidden();
  await expect(overview.locator(".care-overview-destination--activities")).toBeHidden();
  await expect(overview.locator(".care-overview-kicker")).toHaveCount(0);
  ...
  const viewerGeometry = await overview.evaluate((panel) => {
    ...
    const map = panel.querySelector(".care-overview-map")?.getBoundingClientRect();
    return {
      layoutBottom: layout?.bottom || 0,
      lastVisibleBottom: Math.max(map?.bottom || 0, ...cards.map((card) => card.bottom))
    };
  });
  expect(Math.abs(viewerGeometry.layoutBottom - viewerGeometry.lastVisibleBottom)).toBeLessThanOrEqual(2);
...
test("Versorgungs-Kompass: Übersicht bleibt in der kompakten Tabletbreite lesbar", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "chromium-desktop", "Die Zwischenbreite wird einmal im Desktop-Projekt geprüft.");
  await page.setViewportSize({ width: 820, height: 930 });
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#care", { role: "admin" });

  const overview = page.locator('[data-view-panel="careOverview"]');
  const cards = overview.locator(".care-overview-destination:visible");
  ...
  expect(Math.abs(cardRects[0].top - cardRects[1].top)).toBeLessThanOrEqual(2);
  expect(Math.abs(cardRects[2].top - cardRects[3].top)).toBeLessThanOrEqual(2);
...
test("Versorgungs-Kompass: kanonische Übersicht und Karte bleiben getrennte Routen", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "chromium-desktop", "Der Clean-URL-Vertrag wird einmal im Desktop-Projekt geprüft.");
  await installTargetLikeCleanUrlShell(page);
  await gotoAuthenticated(page, "/versorgung", { role: "admin", cleanUrls: true });
...
test("Stakeholder: KVn rendern als Organisationstabelle ohne Listen-Modi", async ({ page }, testInfo) => {
  ...
  await expect(page.locator("#workspace-view-title")).toHaveText("Kassenärztliche Vereinigungen");
  await expect(page).toHaveURL(/#stakeholders\/kv$/);
...
test("Stakeholder: Krankenkassen und aerztliche Verbaende starten nach Mitgliederzahl", async ({ page }) => {
  await gotoAuthenticated(page, "/frontend/app/versorgungs-kompass.html#stakeholders/krankenkassen", { role: "admin" });
  ...
  await expect(page.locator('[data-stakeholder-type-route="health-insurance"]')).toHaveAttribute("aria-current", "page");
  await expect(page.locator("#workspace-view-title")).toHaveText("Krankenkassen");
  await expect(page.locator("#stakeholder-organization-list")).toContainText("Demo-Krankenkasse A");
...
test("Stakeholder: weitere Typen nutzen Organisationstabellen und Profile", async ({ page }) => {
  ...
  await expandStakeholderOrganizationSubnavIfNeeded(page);
  await expect(page.locator('[data-stakeholder-type-route="health-insurance"]')).toBeVisible();
  await expect(page.locator("#stakeholder-mode-actions")).toHaveCount(0);
  await expect(page.locator("#stakeholder-people-table")).toHaveCount(0);
  await expect(page.locator("#stakeholder-map-panel")).toHaveCount(0);
  await page.locator('[data-stakeholder-type-route="health-insurance"]').click();
  await expect(page.locator('[data-stakeholder-type-route="health-insurance"]')).toHaveAttribute("aria-current", "page");
...
test("Stakeholder: Bereich ist im eigenen Sidebar-Abschnitt ohne obere Modus-Reiter erreichbar", async ({ page }) => {
  ...
  const stakeholderLabelsStayInsideButtons = await page.locator('[data-stakeholder-type-route] > span').evaluateAll((labels) =>
    labels.every((label) => {
      ...
      const buttonBounds = label.parentElement.getBoundingClientRect();
