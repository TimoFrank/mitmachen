...
Der produktive Versorgungs-Kompass lädt Kontaktdaten ausschließlich über das geschützte Backend-API. Die früheren öffentlichen CSV/JS-Bestände wurden entfernt, damit GitHub Pages keine echten Kontakt- oder Personendaten ausliefert.
...
Das maschinenlesbare Mapping liegt in `frontend/data/fachrichtungen-zielkatalog.json`. Produktive Kontaktdaten werden weder in GitHub gespeichert noch direkt aus dem Browser abgerufen; die Realanwendung greift ausschließlich über `/api` auf den geschützten Datenspeicher zu.
