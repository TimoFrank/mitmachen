...
```text
http://127.0.0.1:4173/frontend/demo/
```
...
GitHub Actions erzeugt `dist/pages/` aus den fuehrenden Quellen und veroeffentlicht dieses Artefakt direkt. Der Build passt die relativen Karten- und Assetpfade fuer die Pages-Struktur an.
...
Die synthetische Abgrenzung ist technisch pruefbar:

- Datensatz-IDs beginnen mit `demo-`.
- Personen und Teams tragen sichtbar `Demo` im Namen.
- E-Mail-Adressen und Quellenhinweise verwenden ausschliesslich die reservierte, nicht aufloesbare Domain `example.invalid`.
- Profil- und Kontaktbilder sind lokale generische SVG-Dateien; es werden keine externen Profilbilder geladen.
- Ortsangaben und Kartenkoordinaten dienen nur der Darstellung regionaler Demo-Szenarien und sind keiner realen Person oder Organisation zugeordnet.

`npm run check:demo` prueft diese Regeln zusammen mit Anzahl, Struktur und Eindeutigkeit der Demo-Hospitationen.
