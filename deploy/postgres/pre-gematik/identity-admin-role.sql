...
-- owner. Revoking its ACL as the identity-object owner would abort the complete
...
do $pre_gematik_identity_admin_revoke_owned_functions$
declare
...
begin
  ...
  loop
    execute format(
      'revoke all privileges on function %s from vk_identity_admin',
      routine_signature
    );
  end loop;
end
$pre_gematik_identity_admin_revoke_owned_functions$;
