...
resource "google_secret_manager_secret" "password_reset_smtp_password" {
  secret_id = var.PASSWORD_RESET_SMTP_PASSWORD_SECRET_NAME
  ...
  annotations = {
    purpose = "Password-reset SMTP password; value is populated outside Terraform"
  }

  depends_on = [google_project_service.required["secretmanager.googleapis.com"]]
}

resource "google_secret_manager_secret_iam_member" "password_reset_smtp_password_workload" {
  project   = var.GCP_PROJECT_ID
  secret_id = google_secret_manager_secret.password_reset_smtp_password.secret_id
  role      = "roles/secretmanager.secretAccessor"
