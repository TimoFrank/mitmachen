...
if (!csvArgument) {
  console.error("Bitte den Pfad zu einer geschützten CSV-Datei außerhalb des Repositorys explizit angeben.");
  process.exit(1);
}

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const csvPath = path.resolve(csvArgument);
const pathWithinRepository = path.relative(repoRoot, csvPath);
if (pathWithinRepository === "" || (!pathWithinRepository.startsWith("..") && !path.isAbsolute(pathWithinRepository))) {
  console.error("Der Import verweigert Quelldateien im Repository. Nutze einen geschützten Pfad außerhalb des Git-Arbeitsbaums.");
  process.exit(1);
}
if (!fs.existsSync(csvPath) || !fs.statSync(csvPath).isFile()) {
  console.error("Die angegebene geschützte CSV-Datei wurde nicht gefunden.");
  process.exit(1);
}
