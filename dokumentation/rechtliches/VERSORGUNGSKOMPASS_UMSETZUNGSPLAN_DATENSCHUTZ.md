...
Die folgenden als „historischer Providerstand“ bezeichneten Fundstellen halten den am 27. Juli 2026 bewerteten Codezustand als Prüfspur fest. Nach dem Repository-Decommission sind sie nur noch über die Git-Historie verfügbar und keine aktuelle Implementierungsquelle. Für neue Änderungen sind das PostgreSQL-Schema unter `deploy/postgres/`, die API und deren serverseitige Rollenmatrix maßgeblich; offene Tickets müssen gegen diesen Stand neu verifiziert werden.

| Fundstelle | Ist-Befund | Änderungsticket |
|---|---|---|
| Konzeptdemo und realer TYPO3-Intake | Der Supabase-Legacy-Intake ist stillgelegt und aus aktivem Code und Deployment entfernt. Die Repo-Konzeptdemo sendet keinen Request. Der getrennte HMAC-M2M-Connector für das reale Powermail-Formular ist implementiert, aber standardmäßig deaktiviert; er legt weder Kontakte noch Organisationen automatisch an. | Beide Aktivierungsschalter bis zur fachlichen, datenschutzrechtlichen, sicherheitsbezogenen und betrieblichen Freigabe ausgeschaltet lassen; Konzeptdemo weiterhin inert halten |
| Datenschutzhinweis und optionale E-Mail-Kommunikation | Der Connector weist `privacy_notice_version` und `privacy_notice_presented_at` nach. Die bisherige Pflicht-Checkbox zur bloßen Verarbeitung darf nicht als Einwilligung interpretiert werden; eine optionale Kommunikationsanfrage beginnt höchstens als `pending`. Ein produktiver DOI-Versand- und Bestätigungsprozess ist nicht Teil des Connectors. | Live-Formular korrigieren und Textversionen freigeben; DOI mit Token, Versand, Bestätigung, Ablauf, Widerruf und Sperre separat umsetzen, bevor `granted` verwendet wird |
| Intake-Datenhaltung und Berechtigungen | `network_registrations` trennt ungeprüfte Eingänge vom CRM-Bestand. Die `NOLOGIN`-Laufzeitrolle besitzt nur `SELECT` und `INSERT`; Browser erhalten weder eine generische Intake-Route noch Datenbankrechte. Supabase-RLS ist kein aktiver Kontrollpfad mehr. | Minimale Grants, HMAC-/Ingress-Grenze, Zweckbindung, Aufbewahrungsprüfung, Auditierung und negative Berechtigungstests vor Aktivierung live attestieren |
| Historischer Providerstand: `supabase/functions/network-registration/index.ts` | `consent_processing_accepted_at` wird als notwendige Zustimmung verlangt; Fehlermeldung „Die notwendige Verarbeitung wurde nicht bestätigt.“ | Pflichtzustimmung entfernen; stattdessen nur nachweisen, welche Datenschutzhinweis-Version beim Absenden angezeigt wurde |
| Historischer Providerstand: `supabase/schema.sql` – `network_registrations` | `consent_processing_version` und `consent_processing_accepted_at` sind `NOT NULL` | in `privacy_notice_version`/`privacy_notice_presented_at` überführen; keine Einwilligungssemantik für den operativen Zweck |
| Historischer Providerstand: `supabase/schema.sql` – E-Mail-Bestätigung | Statusfelder sind vorhanden, ein vollständiger DOI-Versand- und Bestätigungsworkflow ist im geprüften Code aber nicht erkennbar | Token-, Versand-, Bestätigungs-, Ablauf- und Widerrufsworkflow implementieren; `granted` erst nach Bestätigung |
| Historischer Providerstand: `supabase/schema.sql` – Kontaktstatus | `granted` kann ohne gesonderten `pending`-Status geführt werden | `pending` beziehungsweise getrennte Felder `consent_checked_at` und `consent_confirmed_at` ergänzen |
...
| Historischer Providerstand: `supabase/schema.sql` – RLS | Policy `contacts authenticated read active` erlaubt allen authentifizierten Profilen die Lektüre aller aktiven Kontakte | RLS/API-Scope auf autorisierte Versorgungskompass-Gruppe und erforderliche Zeilen/Felder begrenzen |
