...
Stand: 19.07.2026
Prüfmodus: defensiv; Repository-Prüfung plus bestätigte, eng begrenzte Supabase-Live-Härtung
...
| --- | --- | --- | --- | --- | --- | --- |
| R01 | P0 / kritisch | Supabase-Übergang | Offene Registrierung und ein automatisch aktives Profil konnten den fachlichen Freigabeprozess umgehen. | `[~]` Die restriktiven Active-Profile-, Tabellen- und Storage-Regeln sind live reconciled; neue Profile bleiben im Sollzustand inaktive Viewer. | `supabase/config.toml:4-9`; `supabase/migrations/20260719154214_reconcile_security_and_protected_storage.sql`; Live-Prüfung von RLS, Grants, Funktionen und privaten Buckets. | Auth-Schalter und Nutzer-/Sessioninventar prüfen; negative REST-Tests und die sofortige Sperrwirkung vorhandener JWTs bestätigen. |
| R02 | P0 / kritisch | Supabase-Übergang | Browser konnten eine zu weit freigegebene Notification-RPC missbrauchen. | `[x]` `EXECUTE` ist live für `PUBLIC`, `anon` und `authenticated` entzogen; unsichere Neuerzeugung bleibt deaktiviert. | `supabase/migrations/20260719154214_reconcile_security_and_protected_storage.sql`; Live-Grant- und Funktionsprüfung. | Vor einer späteren Reaktivierung zusätzlich Allowlist-, Empfänger-, Mengen-, Rate- und Idempotenztests ausführen. |
...
| R17 | P1 / mittel | Cloud SQL / Betrieb | Sicherung, Wiederherstellung und Rollback waren nicht ausreichend als Sollzustand verankert. | `[~]` Terraform beschreibt private Cloud SQL, Löschschutz und Sicherungen. Helm unterstützt atomare Rollouts; Datenimport und Software-Release sind getrennt. | `deploy/terraform/gcp-autopilot/sql.tf`; Helm `deploy/helm/versorgungs-kompass/values.yaml`; `SUPABASE_CLOUD_SQL_MIGRATION.md`. | Plattform-Standardbackup und Rücknahme des Software-Releases im jeweiligen PoC prüfen. |
| R18 | P2 / mittel | Repository / Datenschutz | Reale Alias-, E-Mail-, Secret- oder Fachdatenmuster können unbeabsichtigt veröffentlicht werden. | `[~]` Aliasfunktion ist deaktiviert; öffentliche Assets und der aktuelle Repository-Stand werden automatisiert geprüft. | `config/security/gitleaks.toml`; `scripts/audit_public_assets.mjs` | Bei Treffern Veröffentlichung stoppen, Fundstelle prüfen und betroffene Zugangsdaten bei Bedarf ersetzen. |
...
| M02 Notification-RPC sperren | `[x]` | Rechteentzug live reconciled und geprüft. |
...
| M11 RLS-/Grant-/Advisor-Gate | `[~]` | RLS, Grants, Funktionen und private Storage-Grenzen live geprüft; ein planabhängiger Leaked-Password-Advisor sowie die End-to-End-Rollenmatrix bleiben offen. |
...
- [ ] **Wiederherstellung:** geschützte Sicherung, Wiederherstellungstest und erreichbare Administrationskonten dokumentieren.
...
- [~] **Supabase DB/RLS:** Reconciliation live angewendet; Policies, Grants, Functions und Storage inventarisiert; Rollenmatrix noch negativ End-to-End testen und planabhängigen Leaked-Password-Schalter bewerten.
