...
| Supabase-Legacy-Backend | Das Projekt ist pausiert (`INACTIVE`) und nicht gelöscht. Alle 5 Auth-Nutzer sind gesperrt; aktive Sessions und Refresh Tokens stehen jeweils bei 0. Laufzeitcode, Edge Functions und Einmalmigrationen sind aus dem aktiven Repository entfernt. | Nur historische Evidenz und negative Regressionswächter bleiben erhalten. Wiederanlauf erfolgt ausschließlich aus dem 90-Tage-Direktrestore oder dem bis zu einer gesonderten dokumentierten Aufbewahrungs- oder Löschentscheidung gehaltenen Recovery-Archiv und ist kein regulärer Produktionspfad. |
| GKE-Autopilot-Pre-Integration | Container, Helm, NetworkPolicies, Workload Identity, IAP-JWT-Prüfung, Cloud-SQL-Proxy und resiliente Rollouts sind als überprüfte Artefakte vorhanden. Eine Live-Bestandsprüfung bestätigt 0 Supabase-Referenzen in Workloads, Konfiguration, Helm-Historie und Laufzeitlogs. | Die Supabase-Abwesenheit ist attestiert. Die davon unabhängige Abnahme von Ingress, IAP, Cloud SQL und allgemeinen Policywirkungen bleibt ein eigenes Zielbetriebs-Gate. |
...
| --- | --- | --- | --- | --- | --- | --- |
| R01 | P0 / kritisch | Legacy-Backend | Offene Registrierung und ein automatisch aktives Profil konnten den fachlichen Freigabeprozess umgehen. | `[x]` Der Provider ist pausiert; 5/5 Nutzer sind gesperrt, aktive Sessions und Refresh Tokens stehen bei 0. Der aktive Quellbaum und das GKE-Deployment enthalten keinen Supabase-Laufzeitpfad mehr. | Live-Auth-Inventar, Projektstatus `INACTIVE`, GKE-Bestandsaudit und `scripts/test_legacy_backend_decommission.mjs`. | Keine offene Supabase-Abnahme. Eine spätere Wiederherstellung ist ein eigener, expliziter Freigabeprozess und darf den pausierten Legacy-Pfad nicht automatisch reaktivieren. |
| R02 | P0 / kritisch | Legacy-Backend | Browser konnten eine zu weit freigegebene Notification-RPC missbrauchen. | `[x]` Die RPC-/Edge-Laufzeit ist stillgelegt: Projekt `INACTIVE`, Edge-Code aus dem aktiven Repository entfernt und kein Supabase-Endpunkt in GKE. | Projektstatus, GKE-Bestandsaudit und negative Legacy-Backend-Regressionsprüfung. Die frühere Reconciliation bleibt als historische Evidenz in Git erhalten. | Keine offene Supabase-Abnahme; eine spätere fachliche Benachrichtigungsfunktion benötigt einen neuen Cloud-SQL-/API-Vertrag mit eigener Sicherheitsfreigabe. |
...
| R06 | P0 / hoch | Auth-Frontend | Browserseitige Passwort-Hashes, Alias-, Supabase- oder LocalStorage-Fallbacks konnten Authentisierung beziehungsweise Datenverfügbarkeit vortäuschen. | `[x]` Passwort-/Hash-/Aliaslogik wurde entfernt. Das Target besitzt weder Browser-Supabase noch LocalStorage-Fachdaten oder eine LocalStorage-Ersatzsitzung. Zielartefakte akzeptieren nur `iap` oder `oidc`, nutzen fachlich ausschließlich `/api/...` und brechen ohne verifizierte externe Session fail-closed ab. Der frühere Alias-Endpunkt ist durch den pausierten Provider nicht mehr erreichbar. | `frontend/login/auth-guard.js`; `frontend/data/runtime-config.js`; `scripts/prepare_target_frontend_config.mjs`; `scripts/test_security_contracts.mjs`; `scripts/test_legacy_backend_decommission.mjs`; Projektstatus `INACTIVE`. | Target-Negativtests müssen weiterhin bestätigen, dass `401`, `403`, fehlende Routen und API-Ausfälle keine Demo- oder Browser-Speicherdaten aktivieren. Historische, andernorts wiederverwendete Passwortwerte sind bei Unsicherheit gezielt zu rotieren. |
...
| R10 | P0 / hoch | Fachmutationen / Audit | Mehrschrittige Mutationen und Audit konnten auseinanderlaufen oder Client-Actor-Felder übernehmen. | `[~]` Kernpfade für Kontakte, Formate, Beteiligungen, Hospitationen und Beobachtungen laufen transaktional; Activity-Events werden im selben Commit geschrieben. Optimistic Concurrency verhindert Lost Updates. Der Actor stammt aus der verifizierten API-Identität. Der korrigierte Repository-Vertrag gibt der `NOLOGIN`-Runtime auf `activity_events` nur `SELECT`/`INSERT`, nicht `UPDATE`/`DELETE`. | `api/server.mjs`; `deploy/postgres/pre-gematik/runtime-role.sql`; `deploy/postgres/pre-gematik/grants.sql:29-38,91-92`; `scripts/test_security_contracts.mjs`. | Read-only Live-Prüfung am 31.07.2026 zeigt noch effektive `UPDATE`-/`DELETE`-Rechte für `vk_app_runtime`. Den korrigierten Grant-Vertrag nach Backup und geschütztem Apply live abgleichen und erneut attestieren. P2: transaktionale Outbox ergänzen, falls garantierte Benachrichtigungszustellung erforderlich wird. |
...
| R17 | P1 / mittel | Cloud SQL / Betrieb | Sicherung, Wiederherstellung und Rollback waren nicht ausreichend als Sollzustand verankert. | `[~]` Terraform beschreibt private Cloud SQL, Löschschutz und Sicherungen. Helm unterstützt atomare Rollouts; Datenimport und Software-Release sind getrennt. Der gemeinsame Restore-Test ist bestanden; 542 Archivobjekte mit 14.090.082 Bytes sind vollständig event-held. | `deploy/terraform/gcp-autopilot/sql.tf`; Helm `deploy/helm/versorgungs-kompass/values.yaml`; `../betrieb-und-deployment/SUPABASE_CLOUD_SQL_MIGRATION.md`; Restore- und Archivinventar vom 31.07.2026. | Plattform-Standardbackup und Rücknahme des Software-Releases im jeweiligen PoC prüfen; das externe Recovery-Archiv bis zu einer gesonderten dokumentierten Entscheidung erhalten. |
| R18 | P2 / mittel | Repository / Datenschutz | Reale Alias-, E-Mail-, Secret- oder Fachdatenmuster können unbeabsichtigt veröffentlicht werden. | `[~]` Alias-/Edge-Code ist aus dem aktiven Repository entfernt und der Provider pausiert; öffentliche Assets und der aktuelle Repository-Stand werden automatisiert geprüft. Historische Evidenz bleibt für Audit und Secret-Regression erhalten. | `config/security/gitleaks.toml`; `scripts/audit_public_assets.mjs`; `scripts/test_legacy_backend_decommission.mjs` | Bei Treffern Veröffentlichung stoppen, Fundstelle prüfen und betroffene Zugangsdaten bei Bedarf ersetzen. |
...
| **A02 Security Misconfiguration** | `[~]` R01, R02, R09, R13, R15 | Legacy-Provider pausiert und aus Runtime/Deployment entfernt; Uploads aus, gehärtete Helm-Schemas/Pods, exakter Origin, CSP/Headers, Readiness. | Ingress-, allgemeine Cluster- und Headerkontrollen attestieren; die Supabase-Abwesenheit in GKE ist bereits nachgewiesen. |
...
| **A08 Software or Data Integrity Failures** | `[x]` R08, R10, R13, R14 | Appendaudit, atomare Mutationen, immutable Images/Frontend-Releases, Vendor-Hashes, Lockfile-Integrität, Provenance/SBOM und deterministischer Cloud-SQL-Migrationsdigest. | Signatur-/Attestation-Enforcement im Zielbetrieb prüfen; Restore und Legacy-Backend-Rückbau sind nachgewiesen. |
...
| **A10 Mishandling of Exceptional Conditions** | `[~]` R10-R12, R17 | Generische Fehler, Größen-/Zeitlimits, Transaktionsrollback, Graceful Shutdown, Readiness, PDB, sichere Feature-Deaktivierung und bestandener gemeinsamer Restore. | Chaos-/Abbruch-/Last-/Canary-Abnahmen sowie die zielbetriebliche Restore-Wiederholung stehen aus. |
...
| --- | --- | --- |
| M00 Freeze/Backup/Wiederanlauf | `[x]` | Gemeinsamer Restore bestanden; externes Archiv mit 542 Objekten und 14.090.082 Bytes vollständig event-held. Plattformseitiges Direkt-Restorefenster: 90 Tage. |
| M01 Signup schließen | `[x]` | Provider pausiert (`INACTIVE`); 5/5 Nutzer gesperrt, 0 aktive Sessions und 0 aktive Refresh Tokens. |
| M02 Notification-RPC sperren | `[x]` | Legacy-RPC/Edge-Laufzeit stillgelegt; Provider pausiert und Edge-Code aus dem aktiven Repository entfernt. |
| M03 Active-Profile-Gate | `[x]` | Supabase-Profilpfad stillgelegt; die Zielautorisierung erfolgt ausschließlich über API-RBAC und aktive `(issuer, subject)`-Bindings. |
| M04 Nutzer/Profile/Sessions prüfen | `[x]` | Live-Inventar bestätigt 5/5 gesperrte Nutzer sowie 0 Sessions und 0 aktive Refresh Tokens. |
| M05 Sichere Provisionierung | `[x]` | Zielpfad verwendet explizite Identity-Bindings; die stillgelegte Legacy-Provisionierung ist kein Laufzeitpfad mehr. |
| M06 Notifications neu autorisieren | `[x]` | Sicherer Releasezustand durch vollständige Deaktivierung; eine neue Funktion hätte einen eigenen Cloud-SQL-/API-Vertrag und ein separates Abnahmegate. |
| M07 Saved-View-Eskalation | `[x]` | Owner/Scope wird im Zielpfad serverseitig über API-RBAC erzwungen; Legacy-RLS ist kein aktiver Kontrollpfad mehr. |
| M08 Kontakt-Audit atomar | `[~]` | Ziel-API transaktional und Repository-Grant-Vertrag append-only; der Live-Grant-Abgleich für `activity_events` ist nach Backup noch auszuführen und zu attestieren. |
| M09 Hospitations-Actor | `[x]` | Actor wird aus der verifizierten API-Identität abgeleitet und innerhalb der Zieltransaktion geschrieben. |
| M10 Alias-Login | `[x]` | Aliaslogik und Edge-Code aus dem aktiven Repository entfernt; Provider pausiert und 5/5 Alt-Nutzer gesperrt. |
| M11 Legacy-RLS-/Grant-Gate | `[x]` | Historische Supabase-Kontrollen wurden vor der Pause inventarisiert; der aktive Zielpfad nutzt Cloud-SQL-Grants, `NOLOGIN`-Runtime und API-RBAC. Negative Regressionswächter verhindern eine Wiedereinführung. |
...
| M19 Security Headers | `[~]` | Target-Nginx umgesetzt und lokal getestet; Live-Ingress offen. Pages bleibt eine getrennte synthetische Demo und ist kein Ersatz für Target-Header. |
...
- [x] **Wiederherstellung:** Gemeinsamer Restore bestanden; 542 Archivobjekte mit 14.090.082 Bytes vollständig event-held; Archive ohne automatische Löschung. Direktrestore des pausierten Projekts ist plattformseitig 90 Tage möglich.
- [x] **Legacy-Provider:** Supabase-Projekt `INACTIVE` und nicht gelöscht; 5/5 Nutzer gesperrt; 0 Sessions und 0 aktive Refresh Tokens; aktiver Repository- und GKE-Laufzeitpfad frei von Supabase. Historische Evidenz und negative Regressionswächter bleiben erhalten.
...
1. **Legacy-Aufbewahrung:** Pausenstatus und 90-Tage-Direktrestorefenster überwachen; Event-based Holds des Recovery-Archivs erhalten und keine automatische Löschung aktivieren.
2. **Plattform-Trust-Boundary:** gematik-Gateway-, Netzwerk-, TLS- und Tokenvertrag gemeinsam festlegen; Identity selbst bleibt außerhalb dieser Anwendungsprüfung.
3. **Datenpfad:** Cloud-SQL-Transport, `NOLOGIN`-Runtime-Grants, Secrets und `identity_bindings` provisionieren beziehungsweise attestieren; Readiness erst danach grün schalten.
4. **Software Factory:** Branch-/Environment-Gates, signierte immutable Digests, SBOM/Provenance, Scanner und Admission verpflichtend machen.
5. **Staging-Abnahme:** Browserheader, RBAC/Ownership/Archiv, Transaktionen/Audit, Limits, Alerts, Pod-Abbruch und Restore mit synthetischen Daten testen.
6. **Go-live:** dokumentiertes Go/No-Go, Canary, enges Monitoringfenster, kontrollierter Cutover und getesteter Rollback. Der Supabase-Legacypfad bleibt pausiert und aus Repository sowie GKE entfernt; die getrennte Pages-Demo bleibt synthetisch und wird nicht auf Realdaten umgeschaltet.
...
| Playwright `npm run test:visual` | 341 bestanden, 36 bewusst übersprungen, 0 fehlgeschlagen; Desktop, Mobile, Tablet und mehrere Browserprofile |
