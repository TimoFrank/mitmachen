...
| `GET` | `/api/activities/summary` | Exakte Anzahl der für die anfragende Person sichtbaren, normalisierten Aktivitäten im angegebenen Zeitraum laden; `from` ist verpflichtend, `to` optional, maximal 31 Tage |
...
`GET /api/activities/summary?from=<ISO-Zeitpunkt>&to=<ISO-Zeitpunkt>` verwendet exakt denselben Quellenaufbau, dieselbe Rollen-, Archiv- und EHC-Sichtbarkeit sowie dieselbe Kanonisch-/Legacy-Deduplizierung wie die Liste. Fehlt `to`, setzt der Server den aktuellen Zeitpunkt. Der Zeitraum ist auf 31 Tage begrenzt; fehlende, ungültige, zukünftige oder umgekehrt geordnete Grenzen werden mit Status 400 abgewiesen. Die Antwort lautet `{ "count": 42, "from": "...", "to": "..." }` und enthält weder Aktivitätsinhalte noch personenbezogene Daten. Der Server zählt alle gefilterten Quellen vollständig und seitenweise; der Browser lädt dafür keine Aktivitätsseiten. Fehlt in einer Umgebung noch `activity_events`, bleibt der sichtbare Legacy-Verlauf aus `changes` Bestandteil der Zahl.
