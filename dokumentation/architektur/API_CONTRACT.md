...
Ein erfolgreicher Stand wird sechs Stunden im API-Prozess zwischengespeichert. Bei einem späteren Ausfall darf der letzte validierte Stand höchstens 24 Stunden nach seinem Abruf mit `stale: true` ausgeliefert werden; danach endet der Abruf fail-closed. Die Antwort enthält Ausschuss, Wahlperiode, Mitgliedschaftsart, offizielle Quellen-URLs, Abrufzeit, Mitgliederzahl und je Mitglied die validierten Felder `id`, `name`, `party`, `role`, `profileUrl`, `imageUrl`, `imageSourceUrl`, `imageAttribution`, `imageRightsStatus`, `imageUsageTermsUrl`, `constituency`, `constituencyNumber`, `constituencyName`, `constituencyFederalState`, `mandateType`, `constituencyPostalCodes`, `postalCodes`, `postalCodeCoverage` und `constituencySourceUrl`. Die Wahlkreis-PLZ sind die deduplizierte, sortierte Vereinigung aus `counties[].zipCodes[]` und `counties[].communities[].zipCodes[]`; für ein reines Landeslistenmandat ohne ausgewiesenen Wahlkreis bleiben Wahlkreis und PLZ leer. `postalCodes` ist der kompatible Alias für `constituencyPostalCodes`. Bildrechte werden nicht als freigegeben behauptet: `imageRightsStatus` lautet `review_required`, der offizielle Urheberhinweis wird unverändert als Quellenangabe geliefert und `imageUsageTermsUrl` verweist auf die Nutzungsbedingungen im Bundestags-Impressum. Das Frontend zeigt Quelle und Nachweis, lädt das Portrait aber ausschließlich bei `imageRightsStatus: "approved"`; so entstehen vor der Freigabe weder eine Bildnutzung noch ein direkter Drittanbieterabruf aus dem Browser. Die öffentliche Pages-Demo ersetzt diesen Abruf durch `{ "available": false, "demo": true, "members": [] }` und enthält damit keine realen Personendaten.
