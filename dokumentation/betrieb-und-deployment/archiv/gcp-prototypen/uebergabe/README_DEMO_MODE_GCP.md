...
Archivhinweis: Dieser reduzierte GCP-Demo-Deploypfad ist nicht mehr Teil der aktiven Repository-Wurzel. Die zugehoerigen Dateien liegen unter `../statische-demo/`. Fuer den aktuellen Einstieg zuerst `../../../DEPLOYMENT_UEBERSICHT.md` lesen.

Die historische Uebergabe an die Organisations-IT steht in `ORGANISATIONS_GCP_MIGRATIONSPAKET.md`. Kurzfassung, Voraussetzungen, verwendete Dienste, Datenablage und Gespraechsformulierungen stehen in `GCP_ORG_TRANSFER_NOTES.md`. Das alte GCP-Backend-Zielbild steht in `GCP_BACKEND_TARGET.md`. Die einzelnen privaten Testschritte liegen unter `../protokoll/`.
